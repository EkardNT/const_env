...
pub fn env_lit(tokens: TokenStream, read_env: impl ReadEnv) -> TokenStream {
    ...
    let env_var_value_tokens = match env_var_value.parse::<TokenStream>() {
        Ok(tokens) => tokens,
        Err(err) => {
            return syn::Error::new(input.env_var_name.span(), format!("{}", err))
                .to_compile_error()
        }
    };
    ...
    match input.default_value {
        syn::Expr::Lit(syn::ExprLit {
            ...
            match syn::parse_str::<syn::LitStr>(&quoted) {
                ...
                Err(err) => syn::Error::new(
                    input.env_var_name.span(),
                    format!("Invalid string literal contents: {}", err),
                )
                .to_compile_error(),
            }
        }
        syn::Expr::Lit(syn::ExprLit {
            ...
            match syn::parse_str::<syn::LitByteStr>(&quoted) {
                ...
                Err(err) => syn::Error::new(
                    input.env_var_name.span(),
                    format!("Invalid byte string literal contents: {}", err),
                )
                .to_compile_error(),
            }
        }
        syn::Expr::Lit(syn::ExprLit {
            ...
            match syn::parse_str::<syn::LitChar>(&quoted) {
                ...
                Err(err) => syn::Error::new(
                    input.env_var_name.span(),
                    format!("Invalid char literal contents: {}", err),
                )
                .to_compile_error(),
            }
        }
        syn::Expr::Lit(syn::ExprLit {
            ...
            match syn::parse_str::<syn::LitByte>(&quoted) {
                ...
                Err(err) => syn::Error::new(
                    input.env_var_name.span(),
                    format!("Invalid byte literal contents: {}", err),
                )
                .to_compile_error(),
            }
        }
...
fn extract_var_name(attr: TokenStream, default: String) -> String {
    ...
    let expr: Expr = syn::parse2(attr).expect("Unable to parse attribute args as expression");
...
fn extract_var_name_from_expr(expr: &Expr) -> String {
    match expr {
        ...
        Expr::Paren(paren) => extract_var_name_from_expr(&paren.expr),
        _ => {
            panic!(
                "Attribute arguments are not a valid string literal expression: {:?}",
                expr
            )
        }
    }
...
fn value_to_literal(value: &str, original_expr: &Expr) -> Expr {
    match original_expr {
        Expr::Array(_) => syn::Expr::Array(
            syn::parse_str::<syn::ExprArray>(value)
                .expect("Failed to parse environment variable contents as valid array"),
        ),
        Expr::Unary(_) => {
            ...
            let new_lit = match &literal.lit {
                ...
                Lit::ByteStr(original) => {
                    let mut new: syn::LitByteStr = syn::parse_str(&format!("b\"{}\"", value))
                        .expect(
                            "Failed to parse environment variable contents as literal byte string",
                        );
                    new.set_span(original.span());
                    Lit::ByteStr(new)
                }
                ...
                Lit::Char(original) => {
                    let mut new: syn::LitChar = syn::parse_str(&format!("'{}'", value)).expect(
                        "Failed to parse environment variable contents as literal character",
                    );
                    new.set_span(original.span());
                    Lit::Char(new)
                }
                ...
                Lit::Bool(_) | Lit::Int(_) | Lit::Float(_) | Lit::Verbatim(_) => {
                    let new: Expr = syn::parse_str(value).expect(
                        "Failed to parse environment variable contents as valid expression",
                    );
                    return new;
                }
